...
async function refresh() {
  if (props.type) {
    if (Number.parseInt(props.type) === AddressType.RECEIVE) {
      source.value = useStore.receiver
      uni.setNavigationBarTitle({
        title: '收件地址',
      })
    }
    else {
      source.value = useStore.sender
      uni.setNavigationBarTitle({
        title: '寄件地址',
      })
    }
...
function submit() {
  const callback = async (valid: boolean) => {
    if (valid) {
      ...
      try {
        // 直接设置到寄件页
        useStore.setAddress(model, Number.parseInt(props.type))
        router.back()
      }
      catch (err: any) {
      ...
      <div class="flex flex-col gap-3">
        <div class="flex flex-col gap-3 card p-2 py-3">
          ...
          <div class="rounded bg-muted">
            <TnInput
              v-model="pasteText" :height="env.platform === AppPlatform.MP_ALIPAY ? 140 : 120"
              type="textarea" placeholder="请粘贴或输入文本，点击“识别”自动识别姓名、电话和地址" :border="false" clearable
            />
          </div>
        ...
        <view class="card px-5 py-3">
          <TnForm
            ...
            hide-required-asterisk :label-width="env.platform === AppPlatform.MP_ALIPAY ? 135 : 120"
            size="lg" class="text-4"
          >
            <TnFormItem ref="formItemRef" label="姓名" prop="name">
              <div class="flex items-center">
                <MLInput
                  v-model="model.name" type="textarea" placeholder="(选填)请输入姓名" underline
                  class="flex-1"
                />
                <div
                  v-if="env.platform !== AppPlatform.H5"
                  class="flex items-center gap-1 text-3.5 text-muted-foreground"
                  @click="importAddress"
                >
            ...
            <TnFormItem v-if="phoneType === 'mobile'" ref="formItemRef" label="手机号" prop="mobile">
              <div class="flex items-center">
                <MLInput
                  v-model="model.mobile" type="textarea" placeholder="(选填)请输入手机号" underline
                  class="flex-1"
                />
            ...
            <TnFormItem v-if="phoneType === 'landline'" ref="formItemRef" label="座机号" prop="phone">
              <div class="flex items-center">
                <MLInput v-model="model.phone" placeholder="(选填)请输入座机号" underline />
            ...
            <TnFormItem ref="formItemRef" label="地址" prop="address">
              <MLInput
                v-model="model.address" placeholder="(选填)请填写详细地址" underline type="textarea"
                height="150"
              />
            </TnFormItem>
          </TnForm>
